					...
					<div class="borderedTable">
						<div class="table-scrollable">
							<table class="table table-bordered table-hover table table-bordered">
								...
								<tbody>
								<?php
									$json_brands = file_get_contents(SERVER_API . 'Proforms/load_detail_parts_tmp/'.$_COOKIE['id_user']);
									$datos_brands = json_decode($json_brands);
									$brands = $datos_brands->data;
									$contador = 1;
									foreach($brands as $rows):
										$total_rep = number_format($rows->DET_CANTIDAD * $rows->REP_PRECIO, 2,'.', '');
										$sumador_total+=$total_rep;//Sumador
								?>
									<tr>
										<td class='text-center'><?php echo $contador;?></td> 
										<td class='text-left'><?php echo $rows->REP_DESCRIPCION;?></td> 
										<td class='text-center'><?php echo $rows->DET_CANTIDAD;?></td>
										<td class='text-right'><?php echo $rows->REP_PRECIO;?></td>
										<td class='text-right'><?php echo $total_rep;?></td>
										<td class='text-center'><a href="#" class="btn-delete-detail" onclick="eliminar_detalle_tmp('<?php echo $rows->DET_ID ?>', 'proforma_tmp')"><i class="glyphicon glyphicon-trash"></i></a></td>
									</tr>	
								...
								<tbody>
									<?php
										$json_brands = file_get_contents(SERVER_API . 'Proforms/load_detail_lubricants_tmp/'.$_COOKIE['id_user']);
										$datos_brands = json_decode($json_brands);
										$brands = $datos_brands->data;
										$contador = 1;
										foreach($brands as $rows):
										$total_lub = number_format($rows->DET_CANTIDAD * $rows->LUB_PRECIO, 2,'.', '');
								...
								<tbody>
									<?php
										$json_brands = file_get_contents(SERVER_API . 'Proforms/load_detail_labuor_tmp/'.$_COOKIE['id_user']);
										$datos_brands = json_decode($json_brands);
										$brands = $datos_brands->data;
										$contador = 1;
										foreach($brands as $rows):
										$total_man = number_format($rows->DET_CANTIDAD * $rows->MAN_PRECIO, 2,'.', '');
