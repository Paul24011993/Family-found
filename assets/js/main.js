(function($) {
   ...
   // console.log(scrollH - 45);
    ...
    // validar longitud de input random 6 numeros
    $('input#random')
    ...
    /* jQuery plugin which add loading indicators into buttons */
...
function send_email(arg) {
    $.get(atob(DIR_CONTROLLERS) + "sendEmailController.php", arg , function (data) {
        ...
        if(true == data_server){
            swal({
                title: arg.title_swal,
                text: arg.text_swal,
                imageUrl: SERVER_URL + 'assets/images/send_email.gif'
              });
            $('button.has-spinner').buttonLoader('stop');
            //console.log(data_server);
            return data_server;
        }else{
            swal("Error al enviar el correo electrónico: " + data_server );
            return false;
        }
    });
}